...
export const getPins = async (region: string, pixelRatio = 1) => {
  const regionalIcons = await Promise.all(Object.keys(icons).filter(i => i.includes(region)).map(async i => ({
    id: i.split(region + '-')[1],
    svg: (await icons[i]() as { default: string }).default,
  })))

  const regionalIconsAsPng: string[] = await Promise.all(regionalIcons.map(async i => {
